...
AS
BEGIN
	SELECT DISTINCT MH.MaMonHoc, MH.TenMonHoc, MH.HeSo FROM MONHOC MH, XEPLICH XL 
	WHERE MH.MaMonHoc = XL.MaMonHoc AND XL.MaNamHoc = @maNamHoc
END
GO
...
AS
BEGIN
	SELECT MH.MaMonHoc, MH.TenMonHoc, MH.HeSo FROM MONHOC MH, XEPLICH XL  
	WHERE MH.MaMonHoc = XL.MaMonHoc AND XL.MaNamHoc = @maNamHoc AND XL.MaLop = @maLop
END
GO
...
AS
BEGIN
	INSERT INTO XEPLICH (MaNamHoc, MaLop, MaMonHoc )
	VALUES (@maNamHoc, @maLop, @maMonHoc )
END
GO
...
AS
BEGIN
	INSERT INTO LOP (MaLop, TenLop, MaKhoiLop, MaNamHoc, SiSo)
	VALUES (@maLop, @tenLop, @maKhoiLop, @maNamHoc, @siSo)
END
GO
